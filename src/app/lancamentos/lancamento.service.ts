...
export class LancamentoService {
  ...
  // headers.append('Authorization', 'Basic YWRtaW5AYWxnYW1vbmV5LmNvbTphZG1pbg==');
    ...
    if (filtro.dataVencimentoInicio) {
       params.set('dataVencimentoDe',
          moment(filtro.dataVencimentoInicio)
          .format('YYYY-MM-DD'));
    }
    if (filtro.dataVencimentoFim) {
      params.set('dataVencimentoAte',
        moment(filtro.dataVencimentoFim)
        .format('YYYY-MM-DD'));
    }
  ...
  atualizar(lancamento: Lancamento): Promise<Lancamento> {
    ...
    return this.http.put(`${this.lancamentosUrl}/${lancamento.codigo}`, JSON.stringify(lancamento))
      .toPromise()
      .then(response => {
        const lancamentoAlterar = response.json() as Lancamento;

        this.converterStringParaDatas([lancamentoAlterar]);
